...
public class txtIO {
	 ...
	 * one input source to another. This fixes a bug that allows input from the
	 * previous input source to be read after the new source has been selected.
	 */
	...
	/**
	 * The value returned by the peek() method when the input is at end-of-line. The
	 * value of this constant is the character '\n'.
	 */
	public final static char EOLN = '\n'; // The value returned by peek() when at end-of-line.

	/**
	 * After this method is called, input will be read from standard input (as it is
	 * in the default state). If a file or stream was previously the input source,
	 * that file or stream is closed.
	 */
	 ...
	 * is non-null. If inputStream is null, then this method has the same effect as
	 * calling readStandardInput(); that is, future input will come from the
	 ...
	 * has the same effect as calling readStandardInput(); that is, input will be
	 * read from standard input. If an error occurs while trying to open the file,
	 * an exception of type IllegalArgumentException is thrown, and the input source
	 * is not changed. If the file is opened successfully, then after this method is
	 * called, all of the input routines will read from the file, instead of from
	 * standard input.
	 */
	 ...
	 * select an input file. If the user cancels the dialog instead of selecting a
	 * file, it is not considered an error, but the return value of the subroutine
	 * is false. If the user does select a file, but there is an error while trying
	 * to open the file, then an exception of type IllegalArgumentException is
	 * thrown. Finally, if the user selects a file and it is successfully opened,
	 * then the return value of the subroutine is true, and the input routines will
	 * read from the file, instead of from standard input. If the user cancels, or
	 * if any error occurs, then the previous input source is not changed.
	 * <p>
	 * NOTE: Calling this method starts a GUI user interface thread, which can
	 * continue to run even if the thread that runs the main program ends. If you
	 * use this method in a non-GUI program, it might be necessary to call
	 * System.exit(0) at the end of the main() routine to shut down the Java virtual
	 * machine completely.
	 */
	 ...
	 * is non-null. If outputStream is null, then this method has the same effect as
	 * calling writeStandardOutput(); that is, future output will be sent to the
	 ...
	 * has the same effect as calling writeStandardOutput(); that is, output will be
	 * sent to standard output. If an error occurs while trying to open the file, an
	 * exception of type IllegalArgumentException is thrown. If the file is opened
	 * successfully, then after this method is called, all of the output routines
	 * will write to the file, instead of to standard output. If an error occurs,
	 * the output destination is not changed.
	 * <p>
	 * NOTE: Calling this method starts a GUI user interface thread, which can
	 * continue to run even if the thread that runs the main program ends. If you
	 * use this method in a non-GUI program, it might be necessary to call
	...
	/**
	 * Puts a GUI file-selection dialog box on the screen in which the user can
	 * select an output file. If the user cancels the dialog instead of selecting a
	 * file, it is not considered an error, but the return value of the subroutine
	 * is false. If the user does select a file, but there is an error while trying
	 * to open the file, then an exception of type IllegalArgumentException is
	 * thrown. Finally, if the user selects a file and it is successfully opened,
	 * then the return value of the subroutine is true, and the output routines will
	 * write to the file, instead of to standard output. If the user cancels, or if
	 * an error occurs, then the current output destination is not changed.
	 */
	 ...
	 * of the file. If the class is reading from standard input or from a stream,
	 ...
	 * of the file. If the class is writing to standard output or to a stream, then
	 ...
	 * format and no extra spaces. This method will handle any type of parameter,
	 * even one whose type is one of the primitive types.
	 */
	...
	/**
	 * Write a single value to the current output destination, using the default
	 * format and outputting at least minChars characters (with extra spaces added
	 * before the output value if necessary). This method will handle any type of
	 ...
	 * has the same function as System.out.printf(); the details of formatted output
	 * are not discussed here. The first parameter is a string that describes the
	 * format of the output. There can be any number of additional parameters; these
	 * specify the values to be output and can be of any type. This method will
	 * throw an IllegalArgumentException if the format string is null or if the
	 ...
	 * end-of-line. Note that this method does NOT skip whitespace before testing
	 * for end-of-line -- if you want to do that, call skipBlanks() first.
	 */
	...
	/**
	 * Test whether the next character in the current input source is an
	 * end-of-file. Note that this method does NOT skip whitespace before testing
	 * for end-of-line -- if you want to do that, call skipBlanks() or
	 ...
	 * a whitespace character; compare this to the getChar() method, which skips
	 * over whitespace and returns the next non-whitespace character. An end-of-line
	 * is always returned as the character '\n', even when the actual end-of-line in
	 * the input source is something else, such as '\r' or "\r\n". This method will
	 * throw an IllegalArgumentException if the input is at end-of-file (which will
	 ...
	 * error. (Ordinarily, end-of-file is not possible when reading from standard
	 * input.)
	 */
	 ...
	 * method is called, the next input character is either an end-of-file or a
	 * non-whitespace character. This method never causes an error. (Ordinarily,
	 * end-of-file is not possible when reading from standard input.)
	 */
	 ...
	 * end-of-line character, if any). When using standard IO, this will not produce
	 * an error; the user will be prompted repeatedly for input until a legal value
	 ...
	 * discarding the rest of the current line of input (including the next
	 * end-of-line character, if any). Note that the value that is returned will be
	 * a non-whitespace character; compare this with the getAnyChar() method. When
	 * using standard IO, this will not produce an error. In other cases, an error
	 * can occur if an end-of-file is encountered.
	 */
	 ...
	 * input, discarding the rest of the current line of input (including the next
	 * end-of-line character, if any). When using standard IO, this will not produce
	 * an error; the user will be prompted repeatedly for input until a legal value
	 * is input. In other cases, an IllegalArgumentException will be thrown if a
	 * legal value is not found.
	 ...
	 * read, using the getWord() method, and it must be one of these; note that the
	 * "word" must be terminated by a whitespace character (or end-of-file).
	 */
	 ...
	 * the rest of the current line of input (including the next end-of-line
	 * character, if any). A word is defined as a sequence of non-whitespace
	 * characters (not just letters!). When using standard IO, this will not produce
	 * an error. In other cases, an IllegalArgumentException will be thrown if an
	 ...
	 * end-of-line. The end-of-line is read but is not included in the return value.
	 * Any other whitespace characters on the line are retained, even if they occur
	 * at the start of input. The return value will be an empty string if there are
	 * no characters before the end-of-line. When using standard IO, this will not
	 * produce an error. In other cases, an IllegalArgumentException will be thrown
	...
	/**
	 * Skips whitespace characters and then reads a value of type byte from input.
	 * Any additional characters on the current line of input are retained, and will
	 * be read by the next input operation. When using standard IO, this will not
	 * produce an error; the user will be prompted repeatedly for input until a
	 * legal value is input. In other cases, an IllegalArgumentException will be
	 * thrown if a legal value is not found.
	 */
	 ...
	 * from input. Any additional characters on the current line of input are
	 * retained, and will be read by the next input operation. When using standard
	 * IO, this will not produce an error. In other cases, an
	 ...
	 * additional characters on the current line of input are retained, and will be
	 * read by the next input operation. A word is defined as a sequence of
	 * non-whitespace characters (not just letters!). When using standard IO, this
	 * will not produce an error. In other cases, an IllegalArgumentException will
	 * be thrown if an end-of-file is encountered.
	 */
	...
	/**
	 * Skips whitespace characters and then reads a value of type boolean from
	 * input. Any additional characters on the current line of input are retained,
	 * and will be read by the next input operation. When using standard IO, this
	 * will not produce an error; the user will be prompted repeatedly for input
